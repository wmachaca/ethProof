...
contract SimpleVerifier {
  ...
  function verifySimpleProof(SimpleProof calldata proof) external returns (bool) {
    // Step 1: Prepare account key (same pattern as storage key)
    bytes memory accountKey = abi.encodePacked(keccak256(abi.encodePacked(proof.contractAddress)));

    // Step 2: Get account data from STATE trie to extract storage root
    bytes memory accountRLP = MerkleTrie.get(
      accountKey, // Hashed account address as key
    ...
    // Step 3: Parse account RLP to extract storage root
    ...
    // Step 4: Prepare storage verification data
    ...
    // Step 5: Verify the EXACT expected value exists in STORAGE trie
